...
class InvDynamicsNetwork(nn.Module):
    ...
    def __init__(self, hidden_size=512):
        """
        To improve the inverse dynamics model's performance, a lot of fancy stuffs are added:
...
def main(num_demos: int, num_bc_iters: int, num_evals: int, 
    ...
    # Line 1: Initialize the model M_theta as random approximator
    inv_dyn = InvDynamicsNetwork(hidden_size=512).to(device)
    
    # Train M_theta with initial random data but related to the game environment
    ...
    for iteration in range(num_bco_iters):
        ...
        # Line 10-11: Use M_theta with T_demo to approximate A_demo
        ...
        # Line 12: Improve pi by behavioralCloning(S_demo, A_demo)
